...
class assignment_onlinejudge extends assignment_upload {
	...
	function assignment_onlinejudge($cmid = 'staticonly', $assignment = NULL, $cm = NULL, $course = NULL) {
		...
		if (isset ( $this->assignment->id )) {
			$this->onlinejudge = $DB->get_record ( 'assignment_oj', array (
	...
	function setup_elements(&$mform) {
		...
		if (! empty ( $update )) {
			...
			$onlinejudge = $DB->get_record ( 'assignment_oj', array (
		...
		// ideone.com
		$mform->addElement ( 'text', 'ideoneuser', get_string ( 'ideoneuser', 'assignment_onlinejudge' ), array (
				'size' => 20 
		) );
		...
		$mform->addElement ( 'password', 'ideonepass', get_string ( 'ideonepass', 'assignment_onlinejudge' ), array (
				'size' => 20 
		) );
	...
	function update_instance($assignment) {
		...
		if ($returnid) {
			$onlinejudge = $assignment;
			$old_onlinejudge = $DB->get_record ( 'assignment_oj', array (
	...
	function delete_instance($assignment) {
		...
		foreach ( $submissions as $submission ) {
			if (! $DB->delete_records ( 'assignment_oj_submissions', array (
		...
		if (! $DB->delete_records ( 'assignment_oj_testcases', array (
				'assignment' => $assignment->id 
		) )) {
			return false;
		}
		...
		if (! $DB->delete_records ( 'assignment_oj', array (
				'assignment' => $assignment->id 
		) )) {
			return false;
		}
	...
	function get_testcases() {
		...
		$records = $DB->get_records ( 'assignment_oj_testcases', array (
				'assignment' => $this->assignment->id 
		), 'sortorder ASC' );
		$tests = array ();
	 ...
	 *        	Return the link or print it directly
	 */
	function print_student_answer($userid, $return = false) {
		...
		$statustext = html_writer::tag ( 'span', get_string ( 'status' . $onlinejudge_result->status, 'local_onlinejudge' ), array (
				'class' => $statusstyle 
		) );
	 ...
	 *
	 * @param $userid int
	 *        	optional id of the user. If 0 then $USER->id is used.
	 * @param $return boolean
	 *        	optional defaults to false. If true the list is returned rather than printed
	...
	function view_upload_form() {
		...
		if (empty ( $tests )) {
			...
			echo $OUTPUT->single_button ( new moodle_url ( '/mod/assignment/type/onlinejudge/upload.php', array (
			...
			echo "<div style='margin:20px 15%; width:80%'>";
			echo "<form action='" . $paste_code_url . "' method='post'>";
			echo "<label style='color:gray'>" . get_string ( 'pasteformlabel', 'assignment_onlinejudge' ) . "</label>";
			...
			echo "</textarea><br/>";
			echo "<input type='submit' value='" . get_string ( 'submit', 'assignment_onlinejudge' ) . "'/>";
			echo "</form></div>";
			echo $OUTPUT->box_end ();
		}
	...
	function upload_file($mform, $options) {
		...
		$returnurl = new moodle_url ( '/mod/assignment/view.php', array (
				'id' => $this->cm->id 
		) );
		$submission = $this->get_submission ( $USER->id );
	...
	function view_summary($user = 0, $return = true) {
		...
		if (isset ( $onlinejudge_result->status )) {
			...
			$item = html_writer::tag ( 'span', get_string ( 'status' . $onlinejudge_result->status, 'local_onlinejudge' ), array (
					'class' => $itemstyle 
			) );
		...
		if ($onlinejudge_result->status == ONLINEJUDGE_STATUS_COMPILATION_ERROR) {
			...
			foreach ( $onlinejudge_result->testcases as $case ) {
				if (! is_null ( $case )) {
					...
					// details icon link
					$url = new moodle_url ( '/local/onlinejudge/details.php', array (
							'task' => $case->id,
		...
		$item = format_text ( $item, FORMAT_MOODLE, array (
				'allowid' => true 
		) ); // popup details links require id
		$table->data [] = array (
				$item_name,
				$item 
		);
	...
	/**
	 * return success rate.
	 * return more details if $detail is set
	 */
	...
	function request_judge($submission) {
		...
		$oj = $DB->get_record ( 'assignment_oj', array (
		...
		$DB->set_field ( 'assignment_oj_submissions', 'latest', 0, array (
				'submission' => $submission->id 
		) );
		...
		foreach ( $tests as $test ) {
			...
			try {
				...
				$DB->insert_record ( 'assignment_oj_submissions', array (
						'submission' => $submission->id,
						'testcase' => $test->id,
						'task' => $taskid,
						'latest' => 1 
	...
	function grade_marker($status, $fraction) {
		$grades = array (
				...
				ONLINEJUDGE_STATUS_FAKE_ANSWER => 0,
	...
	function extend_settings_navigation($assignmentnode) {
		...
		if (has_capability ( 'mod/assignment:grade', $PAGE->cm->context )) {
			...
			$string = get_string ( 'statisticstestcase', 'assignment_onlinejudge' );
			$link = $CFG->wwwroot . '/mod/assignment/type/onlinejudge/statisticschart.php?id=' . $this->cm->id;
			$assignmentnode->add ( $string, $link, navigation_node::TYPE_SETTING );
		}
	}
}
...
function onlinejudge_task_judged($task) {
	...
	$sql = 'SELECT s.*
        ...
        WHERE o.task = ?';
	if (! $submission = $DB->get_record_sql ( $sql, array (
			$task->id 
	) )) {
